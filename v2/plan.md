...
### 2026-02-08
**新增内容**：
- 新增 User 服务（`pb/user_service.go`），支持用户查询功能
- 客户端重构，新增 `getUser()` 和 `sayHello()` 函数，实现服务链式调用
- 添加测试分类标签：单元测试使用 `//go:build unit`
- Makefile 新增 `test-integration` 命令用于运行集成测试
...
**实现状态**：✅ **部分完成**（服务端已实现连接复用，客户端未实现）

**已完成内容**（2026-02-08）：
- 服务端单个连接支持多次请求（`trpc/server.go:61-66`）
- 连接在defer中统一关闭，避免重复关闭（`trpc/server.go:57-60`）
- 连接错误时自动断开并清理

**待完成**：
- 客户端连接池实现（当前每次调用仍创建新连接）
- 连接保活机制
- 最大连接数限制
...
**实现状态**：✅ **部分完成**（服务端已完成Read和Unmarshal错误处理）

**已完成内容**（2026-02-08）：
- 服务端 `conn.Read()` 错误处理（`trpc/server.go:74-77`）
- 服务端 `json.Unmarshal()` 错误处理（`trpc/server.go:81-83`）
- 连接生命周期日志记录（`trpc/server.go:56,58`）

**待完成**：
- 客户端网络操作错误处理
- `json.Marshal` 错误处理（服务端第93行、客户端第60行）
- 统一错误码定义
